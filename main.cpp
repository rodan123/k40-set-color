...
int print_status (libusb_device_handle *device);
...
static const char *usage =
	...
	"	-n, --number		Read the current profile number and exit.\n"
...
int main (int argc, char *argv[])
{
	...
	int target = print_status (device);
	...
	struct option longopts[] = {
		...
		{ "number", no_argument, nullptr, NumOpt},
	...
	int opt;
	while (-1 != (opt = getopt_long (argc, argv, "p:m:c:a:nrh", longopts, nullptr))) {
		switch (opt) {
		case 'p':
		case ProfileOpt: {
			char *endptr;
